...
> 负责人：**待分配**　|　工程目录：`goal1_authenticity`

区分真人体 / 假人体 / 非人体，输出检查级 `IsNotHumanBodyProb`

## 快速开始

```bash
cd goal1_authenticity
python train.py --limit 40          # ① 先冒烟：确认数据/依赖就绪（几分钟）
python train.py                     # ② 正式训练（产物在 runs/goal1_authenticity/）
python evaluate.py --ckpt runs/goal1_authenticity/checkpoints/best.pth
```
...
- 数据集只读；缓存写在 `goal1_authenticity/cache/`，各 Goal 物理隔离。
