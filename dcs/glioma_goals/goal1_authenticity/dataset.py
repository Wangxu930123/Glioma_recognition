"""Goal1 Authenticity 的数据集（独立副本，可自由修改）。
...
def build_datasets(cfg: dict, data_root: Path, goal_dir: Path, limit: int = 0,
    ...
    return (
        AuthenticityDataset(train_cases, patch=patch, train=True, common_spacing=common,
                 cache_dir=cache, aug_cfg=cfg, seed=seed),
        AuthenticityDataset(val_cases, patch=patch, train=False, common_spacing=common,
                 cache_dir=cache, aug_cfg=cfg, seed=seed + 1),
    )


class AuthenticityDataset(BaseCaseDataset):
    """Goal1 Authenticity 的数据集。"""
