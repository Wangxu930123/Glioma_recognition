...
> 负责人：**待分配**　|　工程目录：`goal2_stitched`

识别拼接影像，输出检查级 `IsStitchedProb`
...
python train.py                     # ② 正式训练（产物在 runs/goal2_stitched/）
python evaluate.py --ckpt runs/goal2_stitched/checkpoints/best.pth
```
...
- 数据集只读；缓存写在 `goal2_stitched/cache/`，各 Goal 物理隔离。
