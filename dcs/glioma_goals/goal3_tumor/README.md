...
> 负责人：**待分配**　|　工程目录：`goal3_tumor`

区分胶质瘤与非肿瘤性病变，输出 `TumorProbability`（ROC-AUC 评估）
...
python train.py                     # ② 正式训练（产物在 runs/goal3_tumor/）
python evaluate.py --ckpt runs/goal3_tumor/checkpoints/best.pth
```
...
- 数据集只读；缓存写在 `goal3_tumor/cache/`，各 Goal 物理隔离。
...
本地数据**无非肿瘤负样本**，该头无法有效训练，AUC 也无法评估；需补充脑梗死/脑脓肿病例。
