...
> 负责人：**待分配**　|　工程目录：`goal4_diagnosis`

WHO 分级 + 位置/形态/边界 + 坏死囊变出血钙化 + 强化形态 + T2/FLAIR 信号
...
python train.py                     # ② 正式训练（产物在 runs/goal4_diagnosis/）
python evaluate.py --ckpt runs/goal4_diagnosis/checkpoints/best.pth
```
...
- 数据集只读；缓存写在 `goal4_diagnosis/cache/`，各 Goal 物理隔离。
