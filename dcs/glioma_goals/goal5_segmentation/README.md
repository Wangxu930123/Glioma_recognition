...
> 负责人：**待分配**　|　工程目录：`goal5_segmentation`

T1 增强核心区 + Flair/T2 周围总异常区的二值掩膜
...
python train.py                     # ② 正式训练（产物在 runs/goal5_segmentation/）
python evaluate.py --ckpt runs/goal5_segmentation/checkpoints/best.pth
```
...
- 数据集只读；缓存写在 `goal5_segmentation/cache/`，各 Goal 物理隔离。
