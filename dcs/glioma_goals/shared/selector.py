...
def pick_series(study, wanted: Iterable[str]) -> dict:
...
def select_first(study, wanted: Iterable[str]):
    """按 ``wanted`` 的顺序返回第一个命中的序列（用于"必需模态"）。"""
    got = pick_series(study, wanted)
    for w in wanted:
        if w in got:
            return got[w]
    return None
