...
用法：
    python scripts/14_calibrate_thresholds.py --fold 0 [--ckpt ...] [--limit 0] [--write]
"""
...
def main() -> int:
    ...
    paths = load_paths()
    raw = a.ckpt or os.path.join(paths["checkpoints_dir"], f"g4_fold{a.fold}", "best.pth")
    ckpts: list[str] = []
    ...
    print(f"[thr] fold{a.fold} 验证 {len(val)} 例；权重 {len(ckpts)} 个{_tag}", flush=True)
    for _c in ckpts:
